...
import py.com.uber.admin.services.TipoDocumentosService;

@RestController
@RequestMapping("/tipo-documento")
public class TipoDocumentosRESTController {
	@Autowired
	private TipoDocumentosService service;
	@PostMapping(path="/paginado")
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity <?> listarPaginado(@RequestBody PaginadoParam<TipoDocumentos> param){
		try {
			PaginadoResult<TipoDocumentos> lista = service.listar(param);
	...
	@PostMapping("/")
	public  ResponseEntity <?> agregarRecurso (@RequestBody TipoDocumentos param){
		Respuesta<TipoDocumentos> respuesta = new Respuesta<>();

		try {
			TipoDocumentos dato = service.guardar(param);
	...
	@PutMapping("/{id}")
	public  ResponseEntity <?> modificarRecurso (@PathVariable("id")Integer id ,  @RequestBody TipoDocumentos param){
		Respuesta<TipoDocumentos> respuesta = new Respuesta<>();

		try {
			TipoDocumentos dato = service.modificar(param,id);
			respuesta.setDato(dato);
	...
	public  ResponseEntity <?> obtenerRecurso (@PathVariable("id")Integer id ){
		...
		try {
			TipoDocumentos dato = service.obtener(id);
	...
	public  ResponseEntity <?> listar ( ){
		Respuesta<List<TipoDocumentos> > respuesta = new Respuesta<>();
		List<TipoDocumentos> dato = new ArrayList<TipoDocumentos>();
		try {
			dato = service.listar();
			System.out.println(dato);
			if(dato==null) {
	...
	public  ResponseEntity <?> activar (@PathVariable("id")Integer id ){
		...
		try {
			TipoDocumentos dato = service.activar(id);
			respuesta.setDato(dato);
	...
	public  ResponseEntity <?> descativar (@PathVariable("id")Integer id ){
		...
		try {
			TipoDocumentos dato = service.desactivar(id);
			respuesta.setDato(dato);
