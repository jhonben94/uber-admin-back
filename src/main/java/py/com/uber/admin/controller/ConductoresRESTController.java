...
import py.com.uber.admin.services.ConductoresService;

@RestController
@RequestMapping("/conductores")
public class ConductoresRESTController {
	@Autowired
	private ConductoresService service;
	@PostMapping(path="/paginado")
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity <?> listarPaginado(@RequestBody PaginadoParam<Conductores> param){
		try {
			PaginadoResult<Conductores> lista = service.listar(param);
	...
	@PostMapping("/")
	public  ResponseEntity <?> agregarRecurso (@RequestBody Conductores param){
		Respuesta<Conductores> respuesta = new Respuesta<>();

		try {
			Conductores dato = service.guardar(param);
	...
	@PutMapping("/{id}")
	public  ResponseEntity <?> modificarRecurso (@PathVariable("id")Integer id ,  @RequestBody Conductores param){
		Respuesta<Conductores> respuesta = new Respuesta<>();

		try {
			Conductores dato = service.modificar(param,id);
			respuesta.setDato(dato);
	...
	public  ResponseEntity <?> obtenerRecurso (@PathVariable("id")Integer id ){
		...
		try {
			Conductores dato = service.obtener(id);
	...
	public  ResponseEntity <?> listar ( ){
		Respuesta<List<Conductores> > respuesta = new Respuesta<>();
		List<Conductores> dato = new ArrayList<Conductores>();
		try {
			dato = service.listar();
			System.out.println(dato);
			if(dato==null) {
