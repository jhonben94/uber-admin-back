...
import py.com.uber.admin.services.ModelosService;

@RestController
@RequestMapping("/modelos")
public class ModelosRESTController {
	@Autowired
	private ModelosService service;
	@PostMapping(path="/paginado")
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity <?> listarPaginado(@RequestBody PaginadoParam<Modelos> param){
		try {
			PaginadoResult<Modelos> lista = service.listar(param);
	...
	@PostMapping("/")
	public  ResponseEntity <?> agregarRecurso (@RequestBody Modelos param){
		Respuesta<Modelos> respuesta = new Respuesta<>();

		try {
			Modelos dato = service.guardar(param);
	...
	@PutMapping("/{id}")
	public  ResponseEntity <?> modificarRecurso (@PathVariable("id")Integer id ,  @RequestBody Modelos param){
		Respuesta<Modelos> respuesta = new Respuesta<>();

		try {
			Modelos dato = service.modificar(param,id);
			respuesta.setDato(dato);
	...
	public  ResponseEntity <?> obtenerRecurso (@PathVariable("id")Integer id ){
		...
		try {
			Modelos dato = service.obtener(id);
	...
	public  ResponseEntity <?> listar ( ){
		Respuesta<List<Modelos> > respuesta = new Respuesta<>();
		List<Modelos> dato = new ArrayList<Modelos>();
		try {
			dato = service.listar();
			System.out.println(dato);
			if(dato==null) {
	...
	public  ResponseEntity <?> activar (@PathVariable("id")Integer id ){
		...
		try {
			Modelos dato = service.activar(id);
			respuesta.setDato(dato);
	...
	public  ResponseEntity <?> descativar (@PathVariable("id")Integer id ){
		...
		try {
			Modelos dato = service.desactivar(id);
			respuesta.setDato(dato);
